...
function evaluateFormula(formula, cells, visited = new Set()) {
  ...
  // Function formulas: SUM, AVG, AVERAGE, COUNT, MAX, MIN, MEDIAN, MULTIPLY, DIVIDE
...
export default function SpreadsheetEditor({ data, onChange, readOnly = false, onToggleFullscreen, isFullscreen }) {
  const initialData = data || { columns: 5, rows: 10, cells: {}, headers: {}, columnWidths: {}, rowHeights: {} };
  ...
  const triggerSave = useCallback((newCells, newHeaders, cols, rows, colWidths, rHeights) => {
  ...
  }, [onChange, columnWidths, rowHeights]);
  ...
  const handleColumnResize = useCallback((colIndex, e) => {
    ...
    function onMouseMove(ev) {
      ...
      setColumnWidths(prev => ({ ...prev, [colIndex]: Math.max(40, startWidth - delta) })); // RTL: subtract
    }
    function onMouseUp() {
      ...
      setColumnWidths(prev => {
        triggerSave(cells, headers, numCols, numRows, prev, rowHeights);
        return prev;
      });
    }
  ...
  const handleRowResize = useCallback((rowIndex, e) => {
    ...
    function onMouseMove(ev) {
      ...
      setRowHeights(prev => ({ ...prev, [rowIndex]: Math.max(20, startHeight + delta) }));
    }
    function onMouseUp() {
      document.removeEventListener('mousemove', onMouseMove);
      document.removeEventListener('mouseup', onMouseUp);
      setRowHeights(prev => {
        triggerSave(cells, headers, numCols, numRows, columnWidths, prev);
        return prev;
      });
    }
    document.addEventListener('mousemove', onMouseMove);
  ...
  function insertCalcFormula(calcId) {
    ...
    if (selection && (selection.startRow !== selection.endRow || selection.startCol !== selection.endCol)) {
      const startRef = getColLetter(Math.min(selection.startCol, selection.endCol)) +
        (Math.min(selection.startRow, selection.endRow) + 1);
      const endRef = getColLetter(Math.max(selection.startCol, selection.endCol)) +
  ...
  // Get selected cell stats for status bar
  function getSelectionStats() {
    ...
    return { sum: sum.toFixed(2), avg: avg.toFixed(2), count: nums.length };
  }
    ...
    <div className={`spreadsheet-editor ${isFullscreen ? 'spreadsheet-editor--fullscreen' : ''}`}>
      ...
      {!readOnly && (
        <div className="spreadsheet-toolbar">
          ...
          <div style={{ position: 'relative' }}>
            <button className="toolbar-btn" onClick={() => setShowCalcMenu(!showCalcMenu)} title="חישובים">
      ...
      <div className="formula-bar">
        ...
        <input
          ...
          placeholder="נוסחה או ערך... (=SUM, =AVERAGE, =MEDIAN, =MULTIPLY, =DIVIDE)"
                ...
                {Array.from({ length: numCols }, (_, ci) => {
                  ...
                  return (
                    <td
                      ...
                      style={{ width: columnWidths[ci] || 100, height: rowHeights[ri] || 32 }}
                    >
                      {isEditing ? (
                        <input
                          ref={cellInputRef}
                          ...
                          onChange={(e) => setFormulaValue(e.target.value)}
                          onKeyDown={(e) => handleCellKeyDown(e, cellRefStr)}
                          onBlur={() => commitCell(cellRefStr, formulaValue)}
                          autoFocus
                          dir="ltr"
                        />
                      ) : (
